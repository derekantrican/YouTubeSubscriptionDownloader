            ...
            if (!string.IsNullOrWhiteSpace(sub.PlaylistIdToWatch))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        while (!results.Any(p => p.Snippet.PublishedAt < sub.LastVideoPublishDate) && response.NextPageToken != null)
                        {
                            listRequest.PageToken = response.NextPageToken;
                            response = await listRequest.ExecuteAsync();
                            results.AddRange(response.Items);
                    ...
                    results = results.Where(p => p.Snippet.PublishedAt > sub.LastVideoPublishDate).ToList();
